...
def shade(
    ...
    if 'kd_ks_normal' in material:
        ...
        ks_grad = torch.sum(torch.abs(all_tex_jitter[..., -6:-3] - all_tex[..., -6:-3]), dim=-1, keepdim=True) / 3
    else:
    ...
    if bsdf == 'pbr':
        if isinstance(lgt, light.EnvironmentLight):
            shaded_col, spec_col = lgt.shade(gb_pos, gb_normal, kd, ks, view_pos, specular=True)
        else:
            assert False, "Invalid light type"
    elif bsdf == 'diffuse':
        if isinstance(lgt, light.EnvironmentLight):
            shaded_col, spec_col = lgt.shade(gb_pos, gb_normal, kd, ks, view_pos, specular=False)
        else:
            assert False, "Invalid light type"
    elif bsdf == 'normal':
...
def render_layer(
    ...
    hard_mask = torch.clamp(rast_out_s[..., -1:], 0, 1)
    antialias_mask = dr.antialias(
        hard_mask.clone().contiguous(), rast_out_s, v_pos_clip, mesh.t_pos_idx.int())
    buffers['mask'] = antialias_mask

    depth, _ = interpolate(v_pos, rast_out_s, mesh.t_pos_idx.int())
    buffers['depth'] = depth[..., -2:-1]
...
def render_mesh(
    ...
    mtx_in2 = torch.tensor(mtx_in2, dtype=torch.float32, device='cuda') if not torch.is_tensor(mtx_in2) else mtx_in2
    view_pos    = prepare_input_vector(view_pos)

    # clip space transform
    v_pos = ru.xfm_points(mesh.v_pos[None, ...], mtx_in2)
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, v_pos, v_pos_clip), rast)]
...
def render_uv(ctx, mesh, resolution, mlp_texture):
    ...
    mask = (rast[..., -1:] > 0)
    mask = mask.view(resolution[0], resolution[1])
    ...
    all_tex[0][tuple(inpaint_coords.T)] = all_tex[0][tuple(search_coords[indices[:, 0]].T)]
    perturbed_nrm = all_tex[..., -3:]

    return mask, all_tex[..., :-6], all_tex[..., -6:-3], util.safe_normalize(perturbed_nrm)
