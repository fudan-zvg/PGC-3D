    ...
    with open(fn, "w") as f:
        ...
        if material is not None:
            ...
            if 'kd' in material.keys():
                ...
                texture.save_texture2D(os.path.join(folder, 'texture_kd_rgb.png'), material['kd'])
            if 'ks' in material.keys():
                f.write('map_Ks texture_ks.png\n')
                texture.save_texture2D(os.path.join(folder, 'texture_ks.png'), material['ks'])
            if 'normal' in material.keys() and not ('no_perturbed_nrm' in material and material['no_perturbed_nrm']):
