...
class NeuralRender:
    ...
    def render_mesh(
        ...
        mtx_in = mtx_in.unsqueeze(dim=1)
        v_pos = mesh_v_pos_bxnx3 - mtx_in[..., :3, 3]
        v_pos = v_pos @ mtx_in[:, 0, :3, :3]
        v_pos = torch.nn.functional.pad(v_pos, pad=(0, 1), mode='constant', value=1.0)
        ...
        albedo = gb_feat[..., 3:6]
        antialias_albedo = dr.antialias(
            albedo.clone().contiguous(), rast, v_pos_clip,
            mesh_t_pos_idx_fx3)

        depth = gb_feat[..., -2:-1]
        ori_mesh_feature = gb_feat[..., :-4]
        return ori_mesh_feature, antialias_mask, hard_mask, rast, v_pos_clip, mask_pyramid, depth, antialias_albedo
