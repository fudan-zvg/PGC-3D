...
import numpy as np
import os
from nerf.fine.dmtet_utils import get_center_boundary_index
...
class DMTetGeometry:
    def __init__(
            self, grid_res=64, bound=2.0, device='cuda', renderer=None,
            render_type='neural_render', args=None):
        super(DMTetGeometry, self).__init__()
        self.grid_res = grid_res
        self.device = device
        self.args = args
        tets = np.load(os.path.join(os.path.dirname(__file__), 'tets/{}_compress.npz'.format(self.grid_res)))
        ...
        self.verts = (self.verts - mid.unsqueeze(dim=0)) / length * 2 * bound
    ...
    def get_mesh(self, v_deformed_nx3, sdf_n, with_uv=False, indices=None):
        ...
        if with_uv:
            marching_tets_alg = DMTet()
            verts, faces, uvs, uv_idx = marching_tets_alg(v_deformed_nx3, sdf_n, indices)
            # faces = torch.cat(
            #     [faces[:, 0:1],
            #      faces[:, 2:3],
            #      faces[:, 1:2], ], dim=-1)
    ...
    def render_mesh(self, mesh_v_nx3, mesh_f_fx3, mesh_vn_nx3, textures_nx3, camera_mv_bx4x4, resolution=256, hierarchical_mask=False):
        return_value = dict()
